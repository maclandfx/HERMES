"""主进20稳健 叠加维度实验：基准 vs ①成交量放大 ②RS50>3% ③MA多头排列"""
...
t0=time.time()
print(f'=== 主进20稳健 维度叠加实验  区间:{START}→{END}  持仓:{HOLD}天  top_k:{TOPK} ===')
...
def sig_vol(ind):
    """①+成交量放大 量比>1.5"""
    return sig_base(ind)&(ind['vrat']>1.5)

def sig_rs50(ind):
    """②+RS50>3%（近50日涨幅筛选）"""
    return sig_base(ind)&(ind['rs50']>0.03)

def sig_mabull(ind):
    """③+MA多头排列 (C>ma20>ma60>ma120)"""
    return sig_base(ind)&(ind['c']>ind['ma20'])&(ind['ma20']>ind['ma60'])&(ind['ma60']>ind['ma120'])
...
print('维度叠加横向对比')
print('='*75)
print(f'| {"公式":<26} | {"交易":>5} | {"胜率":>5} | {"单均净":>7} | {"年化":>7} |')
print('|'+'-'*28+'|'+'-'*7+'|'+'-'*7+'|'+'-'*9+'|'+'-'*9+'|')
for r in sorted(results,key=lambda x:x['年化净%'],reverse=True):
    print(f'| {r["公式"]:<26} | {r["交易数"]:>5} | {r["胜率"]:>5.1f} | {r["单均净%"]:>7.2f} | {r["年化净%"]:>7.1f} |')
print(f'\n总耗时: {time.time()-t0:.0f}s')

ts=pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')
path=f'D:/Hermes/同中书门下/产出/daily_zj20_dimension_{ts}.md'
with open(path,'w',encoding='utf-8') as f:
    f.write('# 主进20稳健 维度叠加实验报告\n\n')
